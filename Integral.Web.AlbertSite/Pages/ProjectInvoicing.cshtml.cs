...
namespace Integral.Web.PortalSite.Pages_Albert {
  ...
  public class ProjectInvoicing : AppCode.PageBaseClasses.ProjectPageBase {
    ...
    // Build the "Quote select" dropdown HTML used in multiple places on the page.
    public string GetQuoteSelectHtml(string selName) {

      var sb = new StringBuilder();
      sb.Append("<div class=\"quote-select\">");
      sb.Append($"<select name=\"{selName}\" size=\"1\" class=\"w100p\">");
      sb.Append("<option value=\"\">[select quote]</option>");
      foreach (var quote in InvoiceItemsByQuote.Quotes) {
        sb.Append($"<option value=\"{quote.QuoteId}\">");
        sb.Append(quote.QuoteName.LimitLengthTo(50).HTMLEncode());
        sb.Append(" (" + (quote.IsAccepted ? quote.ClientAcceptedAmt : quote.InvoiceItemsAmt).ToString("C", "??") + ")");
        sb.Append("</option>");
      }
      sb.Append("</select>");
      sb.Append("</div>");
    ...
    // Build the "Items to invoice" table rows. Done in code-behind because the
    // original aspx used IterateInvoiceItems(...) with an inline closure.
    public List<string> GetItemsToInvoiceRows() {

      var rows = new List<string>();
      InvoiceItemsByQuote.IterateInvoiceItems((quote, invoice, invItem) => {
        if (invoice.InvoiceId != null) return; // only items without an invoice id

        var sb = new StringBuilder();
        sb.Append($"<tr data-amount=\"{invItem.UnitPrice.ToString("0")}\" data-qty=\"{invItem.Quantity.ToString("0")}\"");
        sb.Append($" data-{FormFields.DeleteInvItemId}=\"{invItem.InvoiceItemId}\">");

        if (CanSubmitInvoice) {
          sb.Append("<td class=\"align-center\">");
          sb.Append(WebHelper.CustomCheckBox(GetItemCheckboxName(invItem.InvoiceItemId), invItem.InvoiceItemId.ToString(), false, null));
          sb.Append("</td>");
        }

        sb.Append($"<td class=\"type-date\"><span class=\"date-value\">{TimeHelper.UtcToAppDefaultTimeZone(invItem.CreatedUtc).ToString("d MMM yyyy")}</span></td>");
        sb.Append($"<td class=\"\">{invItem.Description.HTMLEncode()}</td>");
        sb.Append($"<td class=\"type-money\">{invItem.UnitPrice.ToString("C")}</td>");
        sb.Append($"<td class=\"type-qty\">{invItem.Quantity.ToString()}</td>");
        sb.Append($"<td class=\"type-money\">{(invItem.UnitPrice * invItem.Quantity).ToString("C")}</td>");
        sb.Append($"<td class=\"type-money-xlg\">{invItem.InvoiceItemTotalComponentAllocated.ToString()}</td>");
        sb.Append($"<td class=\"align-center\">{(invItem.GSTApplicable ? "Yes" : "No")}</td>");

        sb.Append("<td>");
        if (CanAddDeleteItems) {
          sb.Append("<button class=\"btnDeleteItem btn btn-xsm\" title='Delete Invoice Item'>" + WebHelper.Icon.Delete + "</button>");
        }
        sb.Append("</td>");
    ...
    void SubmitInvoice(AjaxSubmitHelper ajax) {
      ...
      InvoiceItemsByQuote.IterateInvoiceItems((q, i, ii) => {
        ...
        if (WebHelper.GetFormValue(GetItemCheckboxName(ii.InvoiceItemId)) != null) {
