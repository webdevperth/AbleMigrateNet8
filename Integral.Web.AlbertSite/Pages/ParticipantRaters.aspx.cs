...
namespace Integral.Web.PortalSite.Pages_Albert {
  ...
  public partial class ParticipantRaters : System.Web.UI.Page {
    ...
    protected void Page_Load(object sender, EventArgs e) {
      ...
      if (SurveyInfo.FeedbackOption == DbHelper.AlbertSurveys.FeedbackOptionEnum.NoRaters) {
        WebHelper.Redirect(PathHelper.Pages.ParticipantCompleted(urlSurveyUID, urlPartUID));
        return;
      }
      ...
      if (PastRatersForParticipant != null && PastRatersForParticipant.Count > 0) {
      ...
      if (SystemWeb.IsHttpPost) {
        AjaxSubmitHelper.Process(ajax => {
          ...
          switch (ajax.Action) {
            ...
            case AjaxAction.InviteNewRater:
              InviteNewRater(ajax, coacheeInfo);
              return;

            case AjaxAction.InvitePastRaters:
    ...
    public void InviteNewRater(AjaxSubmitHelper ajax, DbHelper.AlbertCoachees.AlbertCoacheeInfo coacheeInfo) {
      ...
      try {
        ...
        telemetry?.Exception(ex)
          .WithOperation("InviteNewRater_AddRaterToSurvey")
          .FromSession()
          .WithProperty("SurveyId", SurveyInfo.SurveyId)
          .WithProperty("SurveyUID", urlSurveyUID)
          .WithProperty("PartUID", urlPartUID)
          .WithProperty("RaterEmail", emailAddr)
          .Track();
    ...
    public void InvitePastRater(AjaxSubmitHelper ajax, DbHelper.AlbertCoachees.AlbertCoacheeInfo coacheeInfo) {
      ...
      foreach (var rater in ratersToInvite) {
        ...
        try {
          ...
          var telemetry = ServiceLocator.Instance.GetRequiredService<ITelemetryService>();
          telemetry?.Exception(ex)
            .WithOperation("InvitePastRater_AddRaterToSurvey")
            .FromSession()
            .WithProperty("SurveyId", SurveyInfo.SurveyId)
            .WithProperty("SurveyUID", urlSurveyUID)
            .WithProperty("PartUID", urlPartUID)
            .WithProperty("RaterEmail", rater.Email)
            .WithProperty("RaterName", $"{rater.FirstName} {rater.LastName}")
            .Track();
