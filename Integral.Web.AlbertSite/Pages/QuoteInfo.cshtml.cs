...
namespace Integral.Web.PortalSite.Pages_Albert {
  ...
  public class QuoteInfo : AppCode.PageBaseClasses.QuotePageBase {
    ...
    private IActionResult Process() {
      ...
      if (SystemWeb.IsHttpPost) {
        ...
        AjaxSubmitHelper.Process(ajax => {
          ...
          if (SystemWeb.RequestFilesContains("filepond")) {
            UploadPDF();
    ...
    void UploadPDF() {
      ...
      var file = SystemWeb.GetRequestFile("filepond");
      if (file == null) return;
      if (!System.IO.Path.GetExtension(file.FileName).Equals(".pdf", StringComparison.OrdinalIgnoreCase)) {
    ...
    bool GetFormValues_Components(AjaxSubmitHelper ajax, FormValues formValues) {
      ...
      foreach (var key in SystemWeb.GetFormKeys()) {
    ...
    public string GetClientQuoteContentHtml() {
      return $@"
