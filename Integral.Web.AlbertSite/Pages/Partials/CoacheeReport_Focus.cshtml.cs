...
namespace Integral.Web.PortalSite.Page_Partials {
  ...
  public class CoacheeReport_Focus : AppCode.PageBaseClasses.CoacheeReportPartialBase {
    ...
    // Returns the two ordered category buckets ("Highest" and "Lowest") for rendering.
    public List<FocusBox> GetFocusBoxes() {

      var boxes = new List<FocusBox>();

      // "Highest" box.
      List<QuestionInfo> questions;
      string highestTitle;
      string highestClass = "modeHighest";
      if (RowSortBy == SortOptions.Difference) {
        highestTitle = "Largest Overestimates";
        ...
        highestTitle = "Highest Scoring Statements";
        questions = (from q in Questions
                     where q.SortByScore != null
                     orderby q.SortByScore descending
                     select q).Take(ConfigHelper.Reports_FocusTab_RowsPerTable).ToList();
      }
      boxes.Add(new FocusBox() { CategoryName = highestTitle, ClassName = highestClass, Questions = questions });

      // "Lowest" box.
      string lowestTitle;
      string lowestClass = "modeLowest";
      if (RowSortBy == SortOptions.Difference) {
        lowestTitle = "Largest Underestimates";
        questions = (from q in Questions
                     where q.SortByScore < 0 && q.SortByScore >= -ConfigHelper.Reports_FocusMaxDifference
                     orderby q.SortByScore ascending
                     select q).Take(ConfigHelper.Reports_FocusTab_RowsPerTable).ToList();
      } else {
        lowestTitle = "Lowest Scoring Statements";
        questions = (from q in Questions
                     where q.SortByScore != null
                     orderby q.SortByScore ascending
                     select q).Take(ConfigHelper.Reports_FocusTab_RowsPerTable).ToList();
      }
      boxes.Add(new FocusBox() { CategoryName = lowestTitle, ClassName = lowestClass, Questions = questions });
