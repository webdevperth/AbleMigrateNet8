...
namespace Integral.Web.PortalSite.Page_Partials {
  ...
  public class CompanyReport_Focus : AppCode.PageBaseClasses.CompanyReportPartialBase {
    ...
    // Exposes the highest and lowest scoring questions in two sections for Razor consumption.
    public IEnumerable<FocusSection> FocusSections {
      get {
        var highestQuestions = (from q in Questions orderby q.SortByScore descending select q).Take(ConfigHelper.Reports_FocusTab_RowsPerTable).ToList();
        yield return new FocusSection("Highest Scoring Statements", "modeHighest", highestQuestions);

        var lowestQuestions = (from q in Questions orderby q.SortByScore ascending select q).Take(ConfigHelper.Reports_FocusTab_RowsPerTable).ToList();
        yield return new FocusSection("Lowest Scoring Statements", "modeLowest", lowestQuestions);
      }
    }
    ...
    public class FocusSection {
      ...
      public FocusSection(string categoryName, string className, List<QuestionInfo> questions) {
