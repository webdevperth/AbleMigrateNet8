...
namespace Integral.Web.PortalSite.Pages_Albert {
  ...
  public class ProjectCustomise : AppCode.PageBaseClasses.ProjectPageBase {
    ...
    // Produce all reminder cadences as a list for use in cshtml foreach.
    public List<DbHelper.SurveyReminderCadence> GetSurveyReminderCadenceList() {
      var list = new List<DbHelper.SurveyReminderCadence>();
      DbHelper.SurveyReminderCadence.ForEach(rc => { list.Add(rc); return true; });
      return list;
    }

    // Render one cadence radio button row.
    public string CadenceOptionHtml(string fieldName, int projectCadenceId, int optionId, string optionText) {

      string tagId = fieldName + "_" + optionId;
      var sb = new StringBuilder();
      sb.Append("<div class=\"flex gap5\">");
      sb.Append($"<input type=\"radio\" name=\"{fieldName}\" value=\"{optionId}\" id=\"{tagId}\" {(optionId == projectCadenceId ? "checked" : "")} />");
      sb.Append($"<label for=\"{tagId}\">{optionText.HTMLEncode()}</label>");
      sb.Append("</div>");
      return sb.ToString();
    }
