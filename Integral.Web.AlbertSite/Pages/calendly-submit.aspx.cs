...
namespace Integral.Web.PortalSite.Pages_Albert {
  ...
  public partial class CalendlySubmit : System.Web.UI.Page {
    ...
    void Main() {
      ...
      if (payloadId > 0) {
        ...
        if (bodyJson.IsNullOrEmpty()) bodyJson = Request.Form["json"]; // From test form.
      ...
      if (webhookInfo.WebhookType == Calendly.WebhookTypeEnum.ReschedulePart2) {
        ...
        if (ConfigHelper.IsDevServer) Response.Write("Reschedule part 2 - ignoring.");
        return;
      }
      ...
      Response.Write(responseMessage + "<br/>\n");
    }
    ...
    string HandleCoachingSessionBooking(
      ...
      if (webhookInfo.WebhookType == Calendly.WebhookTypeEnum.Create) {
        ...
        if (ConfigHelper.IsDevServer) Response.Write("Found SessionId: " + sessionInfo.SessionId + "<br/>");
    ...
    bool TrySavePayloadJson(string bodyJson, out int payloadId) {
      ...
      try {
        ...
        if (ConfigHelper.IsDevServer) Response.Write("payloadId = " + payloadId + "<br/>");
      } catch (Exception ex) {
        var telemetry = ServiceLocator.Instance.GetRequiredService<ITelemetryService>();
        telemetry?.Exception(ex)
    ...
    string DoReschedule(
      ...
      if (ConfigHelper.IsDevServer) Response.Write("<br/>Reschedule...<br/>");
      ...
      if (ConfigHelper.IsDevServer) {
        Response.Write("Coachee Email: " + webhookInfo.CoacheeEmail + "<br/>");
        Response.Write("Reschedule From: " + rescheduleFromDisplay + "<br/>");
        Response.Write("Reschedule To: " + rescheduleToDisplay + "<br/>");
        Response.Write("Reschedule Reason: " + webhookInfo.ReasonText.HTMLEncode() + "<br/>");
      }
    ...
    void LogError(int payloadId, string bodyJson,
      ...
      if (ConfigHelper.IsDevServer) {
        Response.Write($"{messageHtml}<br/>");
        if (ex != null) Response.Write($"Exception: {ex.Message.HTMLEncode()}<br/>");
      }
